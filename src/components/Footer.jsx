        ...
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: 28, marginBottom: 32 }}>
          ...
          <div>
            ...
            {[["Coffee Origins", "/coffee"], ["Tea", "/tea"], ["Spices", "/spices"], ["Dates", "/dates"], ["Gift Sets", "/gift-sets"]].map(([l, p]) => (
              <Link key={l} to={p} style={{ display: "block", color: "#7A5C42", fontSize: 12, textDecoration: "none", marginBottom: 8 }}>{l}</Link>
            ))}
          </div>
          <div>
            <div style={{ color: "#F5EDE3", fontSize: 14, marginBottom: 14, fontWeight: 700 }}>Support</div>
            {["FAQ", "Shipping Info", "Returns", "Contact Us", "Track Order"].map((l) => (
              <button key={l} style={{ background: "none", border: "none", color: "#7A5C42", fontSize: 12, textAlign: "left", padding: 0, display: "block", marginBottom: 8, cursor: "pointer" }}>{l}</button>
            ))}
          </div>
          <div>
            <div style={{ color: "#F5EDE3", fontSize: 14, marginBottom: 14, fontWeight: 700 }}>Company</div>
            {[["Our Story", "/our-story"], ["Sustainability", "/"], ["Wholesale", "/"], ["Blog", "/"], ["Careers", "/"]].map(([l, p]) => (
              <Link key={l} to={p} style={{ display: "block", color: "#7A5C42", fontSize: 12, textDecoration: "none", marginBottom: 8 }}>{l}</Link>
            ))}
          </div>
        </div>
